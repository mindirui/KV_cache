...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None and hijack_mode != "off" and cache is not None:

            module_name = _CURRENT_ATTN_MODULE_NAME[0]
            timestep = kwargs.get("timestep")

            if module_name is not None and timestep is not None:
                
                # 模式一：缓存当前的K/V
                if hijack_mode == "caching":
                    # print(f"Caching K/V for: {module_name} at t={timestep}") # 调试用
                    cache.save(int(timestep), module_name, key, value)
                
                # 模式二：注入缓存的K/V
                elif hijack_mode == "injecting":
                    cached_kv = cache.load(int(timestep), module_name, hidden_states.device)
                    if cached_kv is not None:
                        # print(f"Injecting K/V for: {module_name} at t={timestep}") # 调试用
                        # 分离CFG
                        k_uncond, k_cond = key.chunk(2)
                        v_uncond, v_cond = value.chunk(2)
                        
                        # 加载并准备参考K/V
                        k_ref = cached_kv['k'].to(k_cond.dtype).unsqueeze(0)
                        v_ref = cached_kv['v'].to(v_cond.dtype).unsqueeze(0)
                        
                        # 只对有条件分支进行拼接
                        k_cond_hijacked = torch.cat([k_cond, k_ref], dim=1)
                        v_cond_hijacked = torch.cat([v_cond, v_ref], dim=1)
                        
                        # 将拼接后的K/V重新组合回原始的key, value张量
                        key = torch.cat([k_uncond, k_cond_hijacked], dim=0)
                        value = torch.cat([v_uncond, v_cond_hijacked], dim=0)
        ...
        # 【新的、更精确的探针注入点】
        # ==========================================================
        # 在将Q,K,V送入底层注意力计算前，捕获它们的快照
        # probe_storage = kwargs.get("probe_storage")
        # probe_id = kwargs.get("probe_id")
        # if probe_storage is not None and probe_id and probe_id not in probe_storage:
                
        #     # ... (从hook获取module_name和timestep的逻辑不变) ...
        #     current_module_name = _CURRENT_ATTN_MODULE_NAME[0]
        #     current_timestep = kwargs.get("timestep")

        #     print(f"--- 探针已在目标点 [t={current_timestep}, mod={current_module_name}] 捕获ID为'{probe_id}'的Q, K, V ---")
        ...
        #     }
        #     # 【关键】向传入的 probe_storage 字典中写入数据
        #     probe_storage[probe_id] = snapshot
...
class XFormersAttnProcessor_copy:
    r"""
    Processor for implementing memory efficient attention using xFormers.
